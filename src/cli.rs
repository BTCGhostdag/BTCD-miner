...
#[derive(Parser, Debug)]
#[clap(name = "BTCD-miner", version, about = "A Karlsen high performance CPU miner", term_width = 0)]
pub struct Opt {
    ...
    #[clap(short = 's', long = "BTCDd-address", default_value = "127.0.0.1", help = "The IP of the BTCDd instance")]
    pub BTCDd_address: String,
    ...
    #[clap(short, long, help = "BTCDd port [default: Mainnet = 42110, Testnet = 16211]")]
    port: Option<u16>,
    ...
    #[clap(
        ...
        help = "Mine even when BTCDd says it is not synced",
        long_help = "Mine even when BTCDd says it is not synced, only useful when passing `--allow-submit-block-when-not-synced` to BTCDd  [default: false]"
    )]
...
impl Opt {
    pub fn process(&mut self) -> Result<(), Error> {
        //self.gpus = None;
        if self.BTCDd_address.is_empty() {
            self.BTCDd_address = "127.0.0.1".to_string();
        }

        if !self.BTCDd_address.contains("://") {
            let port_str = self.port().to_string();
            let (BTCDd, port) = match self.BTCDd_address.contains(':') {
                true => self.BTCDd_address.split_once(':').expect("We checked for `:`"),
                false => (self.BTCDd_address.as_str(), port_str.as_str()),
            };
            self.BTCDd_address = format!("grpc://{}:{}", BTCDd, port);
        }
        log::info!("BTCDd address: {}", self.BTCDd_address);
        ...
        self.devfund_address = String::from("BTCD:qzrq7v5jhsc5znvtfdg6vxg7dz5x8dqe4wrh90jkdnwehp6vr8uj7csdss2l7");
