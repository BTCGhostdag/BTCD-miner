...
```sh
git clone https://github.com/BTCD-network/BTCD-miner
cd BTCD-miner
cargo build --release -p BTCD-miner -p kaspacuda -p kaspaopencl
```
...
### From Binaries
The [release page](https://github.com/tmrlvi/BTCD-miner/releases) includes precompiled binaries for Linux, and Windows (for the GPU version).

### Removing Plugins
To remove a plugin, you simply remove the corresponding `dll`/`so` for the directory of the miner. 

* `libkaspacuda.so`, `libkaspacuda.dll`: Cuda support for BTCD-miner
* `libkaspaopencl.so`, `libkaspaopencl.dll`: OpenCL support for BTCD-miner

# Usage
To start mining, you need to run [BTCDd](https://github.com/BTCD-network/BTCDd) and have an address to send the rewards to.
...
USAGE:
    BTCD-miner [OPTIONS] --mining-address <MINING_ADDRESS>
...
OPTIONS:
    ...
    -h, --help                                             Print help information
        --mine-when-not-synced                             Mine even when BTCDd says it is not synced
    ...
    -p, --port <PORT>                                      BTCDd port [default: Mainnet = 16110, Testnet = 16211]
    -s, --BTCDd-address <BTCDd_ADDRESS>                  The IP of the BTCDd instance [default: 127.0.0.1]
...
`./BTCD-miner --mining-address kaspa:XXXXX`
